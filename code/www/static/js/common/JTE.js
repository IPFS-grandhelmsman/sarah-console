...
;String.prototype.clear_html = function(){return this.replace(/<\/?.+?>/g,"").replace(/&[a-zA-Z]+;/g,"");}
;Array.prototype.quote = function(str) {
    for ( var i = 0; i < this.length; i++ ) {
        this[i] = str+this[i]+str;
    }
    return this;
}
...
;function center(jEle, m_left, m_top)
{
    var e_width = jEle.width(), e_height = jEle.height();
    var w_width = $(window).width(), w_height = $(window).height();
    var t_left  = (w_width  - e_width ) / 2, 
        t_top = (w_height - e_height) / 3;

    var css = {}
    if ( m_left ) css['left'] = t_left + 'px';
    if ( m_top  ) css['top']  = t_top  + 'px';
    jEle.css(css);

    return [e_width, e_height, t_left, t_top];
}
...
var JTE = function()
{
    ...
    o.execute = function(tpl, name)
    {
        ...
        var ret = (function(__G__){
            var r = '';
            try {r = eval('(function(){'+fn_str+'}())');} catch (e) {}
            return r;
        }(this._G));
