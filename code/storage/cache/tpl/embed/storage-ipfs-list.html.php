    ...
    <ul class="file-list" id="file_list">
        <?php foreach($this->_symbol["data"] as $val){?>
            <li class="item float-clear">
                <a class="file-link"  <?php echo $val["mimetype"] == 'file/dir' ? 'href="?'.$val["v_qstr"].'&path='.($_GET['path']??'').'/'.$val["file"].'"' : ''?>>
                    <div>
                        <div class="float-left file-logo file-none
                           <?php echo $val["top_type"] == 'text' ? 'file-text': $val["cls_type"] ?>"></div>
                        <div class="float-left detail">
                            <div class="title" data-vqstr="<?php echo $val["v_qstr"]?>" data-type="<?php echo $val["mimetype"]?>">
                                <span class="file-title"><?php echo $val["file"]?></span>
                                <i class="iconfont iconbianji1 icon edit-title"></i>
                            </div>
                            <div class="hash">
                                <span class="hash-text"><?php echo $val["hash"]?></span>
                                <i class="iconfont iconfuzhi copy-hash"></i>
                            </div>
                        </div>
                        <div class="size float-left"><?php echo $val["size_str"]?></div>
                        <div class="time float-left"><?php echo $val["created_time"]?></div>

                        <div class="option float-right text-right  margin-right-v3" data-hash="<?php echo $val["hash"]?>" data-name="<?php echo $val["file"]?>" data-type="<?php echo $val["mimetype"]?>">
                            <?php  if ($val["mimetype"] != "file/dir") { ?>
                            ...
                            <span class="iconfont iconshanchu1 delete" title="删除" data-vqstr="<?php echo $val["v_qstr"]?>"></span>
                        </div>
...
<script src="/static/js/common/util.js?v=<?php echo $this->_symbol["js_version"]?>"></script>
<script src="/static/js/desktop/link_params.js?v=<?php echo $this->_symbol["js_version"]?>"></script>
<script src="/static/js/embed/buffer.js?v=<?php echo $this->_symbol["js_version"]?>"></script>
<script src="/static/js/embed/ipfs.js?v=<?php echo $this->_symbol["js_version"]?>"></script>
<script src="/static/js/embed/storage_ipfs_list.js"></script>
<script>
    var config = {
        host: "<?php echo $this->_symbol["config"]["host"]?>",
        read_port: "<?php echo $this->_symbol["config"]["read_port"]?>",
        write_port: "<?php echo $this->_symbol["config"]["write_port"]?>",
    };
    console.log("<?php echo $this->_symbol["config"]["host"]?>");
    console.log("<?php echo $this->_symbol["config"]["write_port"]?>");
